...
The REST API documentation can be found on [swagger.ioooooooo](https://swagger.ioooooooo). The full API of this library can be found in [api.md](api.md).
...
```sh
npm install git+ssh://git@github.com:stainless-sdks/miriam-example-2-typescript.git
```
...
1. Via the `MIRIAM_EXAMPLE_2_LOG` environment variable
...
const client = new MiriamExample2({
  logger: logger.child({ name: 'MiriamExample2' }),
...
const client = new MiriamExample2({ fetch });
```
...
```ts
import MiriamExample2 from 'npm:miriam-example-2';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/miriam-example-2-typescript/issues) with questions, bugs, or suggestions.
